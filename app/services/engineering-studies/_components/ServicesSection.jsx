...
import {
  getEngineeringStudiesServiceCard,
  getEngineeringStudiesQualificationStages,
  getHeroEngineeringStudies,
} from "@/requests/services/engineering-studies";
import {
...
export default function ServicesSection() {
  const { data: cardData, isLoading: cardLoading } = useQuery({
    queryKey: ["EngineeringStudiesServiceCard"],
    queryFn: getEngineeringStudiesServiceCard,
  });
  const {
    data: qualificationStagesData,
    isLoading: qualificationStagesLoading,
  } = useQuery({
    queryKey: ["EngineeringStudiesQualificationStages"],
    queryFn: getEngineeringStudiesQualificationStages,
  });
  const { data: heroData, isLoading: heroLoading } = useQuery({
    queryKey: ["HeroEngineeringStudies"],
    queryFn: getHeroEngineeringStudies,
  });
  ...
  return (
    <>
      ...
      <GifBackground className="services-section xl1300-padding services-layout clip">
        ...
        !qualificationStagesData?.engineering_studies_stages_card ? (
          <QualificationStagesSkeleton />
        ) : (
          <QualificationStagesSection
            description={qualificationStagesData.description}
            cards={qualificationStagesData.engineering_studies_stages_card}
          />
        )}
      </GifBackground>
    </>
